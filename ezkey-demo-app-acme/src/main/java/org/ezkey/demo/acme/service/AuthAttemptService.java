 ...
 * <p>
 * Provides high-level operations to list, create, retrieve and delete
 * authentication attempts. This demo service delegates to the Ezkey Admin API using a
 * configured {@link WebClient}.
 * </p>
 *
...
public class AuthAttemptService {
     ...
     * <p>
     * Delegates to the Ezkey Admin API to create an authentication attempt
     * for the specified enrollment. The enrollment must exist and be active.
     * </p>
     *
    ...
    public Mono<AuthAttemptCreateResponseDto> createAuthAttempt(AuthAttemptCreateRequestDto request) {
        ...
        return ezkeyAdminApiClient
                ...
                .doOnSuccess(response -> logger.info("Successfully created authentication attempt with ID: {}", 
                        response.getAuthAttemptId()))
                .doOnError(error -> logger.error("Failed to create authentication attempt for enrollment {}: {}", 
                        request.getEnrollmentId(), error.getMessage()));
    }

    /**
     * Retrieves all authentication attempts.
     * <p>
     * Delegates to the Ezkey Admin API to retrieve all authentication attempts.
     * Returns a reactive stream of authentication attempt DTOs.
     * </p>
     *
    ...
    public Flux<AuthAttemptDto> getAllAuthAttempts() {
        ...
        return ezkeyAdminApiClient
                ...
                .doOnError(error -> logger.error("Failed to retrieve authentication attempts: {}", error.getMessage()));
    }

    /**
     * Retrieves all authentication attempts as a List (blocking).
     * <p>
     * Convenience method that converts the reactive Flux to a blocking List.
     ...
     * <p>
     * Delegates to the Ezkey Admin API to retrieve authentication attempts
    ...
    public Flux<AuthAttemptDto> getAuthAttemptsByEnrollmentId(Integer enrollmentId) {
        ...
        return ezkeyAdminApiClient
                ...
                .retrieve()
                .bodyToFlux(AuthAttemptDto.class)
                .doOnComplete(() -> logger.debug("Successfully retrieved authentication attempts for enrollment: {}", 
                        enrollmentId))
                .doOnError(error -> logger.error("Failed to retrieve authentication attempts for enrollment {}: {}", 
                        enrollmentId, error.getMessage()));
    }
    ...
    public List<AuthAttemptDto> getAuthAttemptsByEnrollmentIdSync(Integer enrollmentId) {
        return getAuthAttemptsByEnrollmentId(enrollmentId)
                .collectList()
                .block(Duration.ofSeconds(10));
    }
     ...
     * <p>
     * Delegates to the Ezkey Admin API to retrieve a specific authentication attempt.
     * </p>
     *
    ...
    public Mono<AuthAttemptDto> getAuthAttemptById(Integer authAttemptId) {
        ...
        return ezkeyAdminApiClient
                ...
                .doOnSuccess(response -> logger.debug("Successfully retrieved authentication attempt with ID: {}", 
                        authAttemptId))
                .doOnError(error -> logger.error("Failed to retrieve authentication attempt with ID {}: {}", 
     ...
     * <p>
     * Delegates to the Ezkey Admin API to delete a specific authentication attempt.
     * </p>
     *
    ...
    public Mono<Void> deleteAuthAttempt(Integer authAttemptId) {
        ...
        return ezkeyAdminApiClient
                ...
                .doOnSuccess(response -> logger.info("Successfully deleted authentication attempt with ID: {}", 
                        authAttemptId))
