 ...
 *
 * <p>This controller provides the web interface for managing Ezkey integrations through a Neo
 * Brutalism styled user interface. It demonstrates the complete integration lifecycle including
 * creation, viewing, and deletion of integrations.
 *
 * <p><b>Integration Management Features:</b>
 *
 * <ul>
 *   <li>List all existing integrations
 *   <li>Create new integrations with validation
 *   <li>View detailed integration information
 *   <li>Delete integrations (demo cleanup)
 * </ul>
 *
 * <p><b>UI Technology Stack:</b>
 *
 * <ul>
 *   <li><b>Templates:</b> Thymeleaf with Neo Brutalism styling
 *   <li><b>Interactivity:</b> htmx for dynamic form submission
 *   <li><b>Validation:</b> Alpine.js for client-side validation
 *   <li><b>Design:</b> ACME brand colors and geometric layouts
 * </ul>
 *
...
public class IntegrationController {
   ...
   *
   * <p>Renders the main integrations page showing all configured integrations in a Neo Brutalism
   * styled table. Users can view details, create new integrations, or delete existing ones from
   * this page.
   *
  ...
  public String listIntegrations(Model model) {
    ...
    try {
      ...
      model.addAttribute(
          "error", "Unable to load integrations. Please check that Ezkey Admin API is running.");
    }
   ...
   *
   * <p>Renders the form for creating new integrations with Neo Brutalism styling and Alpine.js
   * client-side validation. The form uses htmx for dynamic submission without page reload.
   *
   * @param model the Spring MVC model for passing data to the view
   * @return the name of the Thymeleaf template to render
   */
   ...
   *
   * <p>Handles the POST request from the integration creation form, validates the input, creates
   * the integration via the Ezkey Admin API, and redirects to the appropriate page with success or
   * error messages.
   *
  ...
  public String createIntegration(
      @ModelAttribute IntegrationCreateRequestDto integrationRequest,
    ...
    try {
      ...
      IntegrationResponseDto createdIntegration =
          integrationService.createIntegrationSync(integrationRequest);
   ...
   *
   * <p>Renders the integration details page showing complete information about the integration
   * including its configuration, QR code for enrollment, and associated enrollments. Uses Neo
   * Brutalism styling consistent with the application design.
   *
  ...
  public String integrationDetails(@PathVariable("id") Integer integrationId, Model model) {
    ...
    try {
      ...
      if (integration != null) {
        ...
        logger.debug(
            "Displaying details for integration: {}",
   ...
   *
   * <p>Handles the deletion of an integration from the Ezkey system. This is primarily used for
   * demo cleanup purposes. The operation will remove the integration and all associated data.
   *
  ...
  public String deleteIntegration(
      @PathVariable("id") Integer integrationId, RedirectAttributes redirectAttributes) {
   ...
   *
   * <p>This endpoint supports htmx-powered form submission for creating integrations without full
   * page reload. Returns appropriate HTTP status codes and responses for dynamic UI updates.
   *
  ...
  public String createIntegrationHtmx(
      @ModelAttribute IntegrationCreateRequestDto integrationRequest, Model model) {
    logger.debug(
        "Processing HTMX integration creation with logo: {}", integrationRequest.getLogo());
