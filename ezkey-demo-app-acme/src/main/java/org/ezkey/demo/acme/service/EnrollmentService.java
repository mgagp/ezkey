 ...
 * enrollments. This demo service delegates to the Ezkey Admin API using a
...
public class EnrollmentService {
    ...
    public Flux<EnrollmentResponseDto> getAllEnrollments() {
        ...
        return ezkeyAdminApiClient
                ...
                .onErrorResume(WebClientResponseException.class, ex -> {
                    logger.warn("API error fetching enrollments: {} - {}", ex.getStatusCode(), ex.getMessage());
                    return Flux.empty();
                })
    ...
    public Mono<EnrollmentResponseDto> getEnrollmentById(Integer enrollmentId) {
        ...
        return ezkeyAdminApiClient
                ...
                .onErrorResume(WebClientResponseException.class, ex -> {
                    ...
                    logger.warn("API error fetching enrollment {}: {} - {}", enrollmentId, ex.getStatusCode(), ex.getMessage());
                    return Mono.empty();
                })
                .onErrorResume(Exception.class, ex -> {
    ...
    /**
     * Retrieves all enrollments for a given integration by client-side filtering.
     * The Admin API currently exposes only a global list endpoint.
     *
    ...
    public Mono<EnrollmentCreateResponseDto> createEnrollment(EnrollmentCreateRequestDto request) {
        logger.info("Creating new enrollment for integration {} with name {}", request.getIntegrationId(), request.getName());
    ...
    public List<EnrollmentResponseDto> getEnrollmentsByIntegrationSync(Integer integrationId) {
        return getEnrollmentsByIntegration(integrationId)
                .collectList()
