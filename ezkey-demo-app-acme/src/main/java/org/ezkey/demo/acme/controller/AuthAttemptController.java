 ...
 *
 * <p>This controller provides the web interface for creating, monitoring, and managing
 * authentication attempts through a Neo Brutalism styled user interface. It demonstrates the
 * complete authentication flow including attempt creation, real-time status monitoring, and result
 * handling within the ACME demo environment.
 *
 * <p><b>Authentication Flow Features:</b>
 *
 * <ul>
 *   <li>Create authentication attempts for enrolled devices
 *   <li>Real-time monitoring of authentication status
 *   <li>Display authentication results and outcomes
 *   <li>Integration selection and enrollment filtering
 *   <li>Live updates using Server-Sent Events (SSE)
 * </ul>
 *
 * <p><b>Real-time Updates:</b> Implements Server-Sent Events to provide live updates of
 * authentication attempt status changes, creating a responsive user experience for monitoring
 * authentication flows as they occur.
 *
 * <p><b>UI Technology Stack:</b>
 *
 * <ul>
 *   <li><b>Templates:</b> Thymeleaf with Neo Brutalism styling
 *   <li><b>Interactivity:</b> htmx for dynamic updates
 *   <li><b>Real-time:</b> Server-Sent Events for live status
 *   <li><b>Styling:</b> Custom CSS with bold, geometric design
 * </ul>
 *
...
public class AuthAttemptController {
  ...
  public String listAuthAttempts(
    ...
    try {
      ...
      if (enrollmentId != null) {
        ...
        List<AuthAttemptDto> filteredAttempts =
            authAttempts.stream()
      ...
      List<EnrollmentResponseDto> enrollments =
          enrollmentService.getAllEnrollments().collectList().block(Duration.ofSeconds(10));
      ...
      for (AuthAttemptDto attempt : authAttempts) {
        ...
        if (enrollment != null) {
          ...
          enrichedAttempt.put(
              "enrollmentName",
              integrationName != null ? integrationName + " Enrollment" : "Unknown Enrollment");
        } else {
        ...
        String status =
            attempt.getAuthAttemptStatus() != null
                ? attempt.getAuthAttemptStatus().getValue()
                : "PENDING";
      ...
      // If enrollmentId is provided but integrationId is not, try to get integrationId from
      // enrollment
      ...
      if (enrollmentId != null && integrationId == null) {
        try {
          List<EnrollmentResponseDto> enrollments =
              enrollmentService.getAllEnrollments().collectList().block(Duration.ofSeconds(5));
          if (enrollments != null) {
            for (EnrollmentResponseDto enrollment : enrollments) {
  ...
  public String viewAuthAttempt(@PathVariable(value = "id") Integer id, Model model) {
    ...
    try {
      ...
      if (authAttempt != null) {
        ...
        // Use the new authAttemptStatus field directly
        String status =
            authAttempt.getAuthAttemptStatus() != null
                ? authAttempt.getAuthAttemptStatus().getValue()
                : "PENDING";
        enrichedAttempt.put("status", status);
  ...
  public String createAuthAttemptForm(
      @RequestParam(value = "enrollmentId", required = false) Integer enrollmentId, Model model) {
  ...
  public String createAuthAttempt(
    ...
    try {
      ...
      Mono<AuthAttemptCreateResponseDto> responseMono =
          authAttemptService.createAuthAttempt(request);
      AuthAttemptCreateResponseDto response = responseMono.block();

      if (response != null) {
        model.addAttribute(
            "success",
            "Authentication attempt created successfully with ID: " + response.getAuthAttemptId());
        ...
        model.addAttribute(
            "error", "Failed to create authentication attempt: No response from API");
  ...
  public String deleteAuthAttempt(
    ...
    try {
      ...
      if (enrollmentId != null) {
        return "redirect:/auth-attempts?enrollmentId="
            + enrollmentId
   ...
   */
  @GetMapping("/{id}/wait")
  public String showWaitPage(@PathVariable(value = "id") Integer id, Model model) {
    ...
    try {
      ...
      if (enrollment != null && enrollment.getIntegrationId() != null) {
        IntegrationResponseDto integration =
            integrationService.getIntegrationByIdSync(enrollment.getIntegrationId());
        if (integration != null
            && integration.getI18n() != null
            && !integration.getI18n().isEmpty()) {
  ...
  public Map<String, Object> executeWait(
      @PathVariable(value = "id") Integer id, @RequestBody Map<String, Object> request) {
