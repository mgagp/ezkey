...
class AuthAttemptDtoTest{
  ...
  private static final OffsetDateTime TEST_CREATED_AT = OffsetDateTime.of(2025,1,27,10,30,0,0,
      ZoneOffset.ofHours(1));

  private static final OffsetDateTime TEST_EXPIRES_AT = OffsetDateTime.of(2025,1,27,10,35,0,0,
      ZoneOffset.ofHours(1));

  @Test @DisplayName("Should create record with all fields")
  void shouldCreateRecordWithAllFields(){
    // Act
    AuthAttemptDto dto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,TEST_STATUS,
        TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
  ...
  @Test @DisplayName("Should handle null values for optional fields")
  ...
  @Test @DisplayName("Should handle all authentication attempt statuses")
  void shouldHandleAllAuthAttemptStatuses(){
    // Test each status
    for (AuthAttemptStatus status : AuthAttemptStatus.values()){
      AuthAttemptDto dto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,status,
          TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
  ...
  @Test @DisplayName("Should handle edge case values for integer fields")
  void shouldHandleEdgeCaseIntegerValues(){
    // Test with minimum values
    AuthAttemptDto dtoMin = new AuthAttemptDto(Integer.MIN_VALUE,Integer.MIN_VALUE,TEST_STATUS,
        Integer.MIN_VALUE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
    ...
    AuthAttemptDto dtoMax = new AuthAttemptDto(Integer.MAX_VALUE,Integer.MAX_VALUE,TEST_STATUS,
        Integer.MAX_VALUE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
    ...
    // Test with zero
    AuthAttemptDto dtoZero = new AuthAttemptDto(0,0,TEST_STATUS,0,TEST_PROOF_TOKEN,TEST_CREATED_AT,
        TEST_EXPIRES_AT);
  ...
  @Test @DisplayName("Should handle empty and special strings for proof token")
  void shouldHandleSpecialStrings(){
    // Empty string
    AuthAttemptDto dtoEmpty = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        TEST_STATUS,TEST_CHALLENGE,"",TEST_CREATED_AT,TEST_EXPIRES_AT);
    assertThat(dtoEmpty.authAttemptProofToken()).isEmpty();

    // Very long string
    String longToken = "A".repeat(1000);
    AuthAttemptDto dtoLong = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,TEST_STATUS,
        TEST_CHALLENGE,longToken,TEST_CREATED_AT,TEST_EXPIRES_AT);
    assertThat(dtoLong.authAttemptProofToken()).hasSize(1000);
  }

  @Test @DisplayName("Should handle different timezones for timestamps")
  void shouldHandleDifferentTimezones(){
    // UTC timezone
    OffsetDateTime utcTime = OffsetDateTime.of(2025,1,27,10,30,0,0,ZoneOffset.UTC);
    AuthAttemptDto dtoUtc = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,TEST_STATUS,
        TEST_CHALLENGE,TEST_PROOF_TOKEN,utcTime,utcTime);
    assertThat(dtoUtc.createdAt()).isEqualTo(utcTime);
    assertThat(dtoUtc.expiresAt()).isEqualTo(utcTime);

    // Negative offset timezone
    OffsetDateTime negativeOffset = OffsetDateTime.of(2025,1,27,10,30,0,0,ZoneOffset.ofHours(-5));
    AuthAttemptDto dtoNegative = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        TEST_STATUS,TEST_CHALLENGE,TEST_PROOF_TOKEN,negativeOffset,negativeOffset);
  ...
  void shouldVerifyBasicRecordEquality(){
    // Arrange
    AuthAttemptDto dto1 = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,TEST_STATUS,
        TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);

    AuthAttemptDto dto2 = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,TEST_STATUS,
        TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);

    AuthAttemptDto dto3 = new AuthAttemptDto(999,TEST_ENROLLMENT_ID,TEST_STATUS,TEST_CHALLENGE,
        TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
  ...
  @Test @DisplayName("Should verify toString contains all field values")
  ...
  @Test @DisplayName("Should create record with different status values")
  void shouldCreateRecordWithDifferentStatuses(){
    // Test PENDING status
    AuthAttemptDto pendingDto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        AuthAttemptStatus.PENDING,TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
    assertThat(pendingDto.authAttemptStatus()).isEqualTo(AuthAttemptStatus.PENDING);

    // Test ACCEPTED status
    AuthAttemptDto acceptedDto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        AuthAttemptStatus.ACCEPTED,TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
    assertThat(acceptedDto.authAttemptStatus()).isEqualTo(AuthAttemptStatus.ACCEPTED);

    // Test REJECTED status
    AuthAttemptDto rejectedDto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        AuthAttemptStatus.REJECTED,TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
    assertThat(rejectedDto.authAttemptStatus()).isEqualTo(AuthAttemptStatus.REJECTED);

    // Test EXPIRED status
    AuthAttemptDto expiredDto = new AuthAttemptDto(TEST_AUTH_ATTEMPT_ID,TEST_ENROLLMENT_ID,
        AuthAttemptStatus.EXPIRED,TEST_CHALLENGE,TEST_PROOF_TOKEN,TEST_CREATED_AT,TEST_EXPIRES_AT);
