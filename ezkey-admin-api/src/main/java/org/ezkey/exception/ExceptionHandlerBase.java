...
/**
 * Base class for exception handlers providing shared RFC 9457 ProblemDetail construction.
 *
 * <p><b>Purpose:</b> This abstract base class eliminates code duplication across exception handlers
 * by providing a centralized method for building RFC 9457 ProblemDetail responses. Each exception
 * handler extends this class and uses {@link #buildProblemDetail} to construct standardized error
 * responses.
 *
 * <p><b>RFC 9457 Compliance:</b> All responses built by this class conform to RFC 9457 (Problem
 ...
 * </ul>
 *
 * <p><b>Example Usage:</b> Subclasses implement exception handler methods annotated with
 * {@code @ExceptionHandler} and delegate response construction to {@link #buildProblemDetail},
 * passing the exception, HTTP status, problem type URI, human-readable title, and request object.
 *
 * <p><b>Design Pattern:</b> This implements the Template Method pattern, allowing subclasses to
 * focus on exception-to-status mapping while delegating response construction to the base class.
 *
...
public abstract class ExceptionHandlerBase {
   ...
   * </ul>
   *
   * <p><b>RFC 9457 Structure:</b> The returned ProblemDetail conforms to RFC 9457 and includes the
   * following fields: type URI (for error categorization), title (human-readable error category),
   * status (HTTP status code), detail (specific error message), and path (request URI where error
   * occurred).
   *
   * @param ex exception containing the error message (detail)
   * @param status HTTP status code for the response (e.g., HttpStatus.BAD_REQUEST)
   ...
   * @return ResponseEntity containing the ProblemDetail and HTTP status, ready to send to client
