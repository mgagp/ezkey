 ...
 * to the admin login endpoint to prevent brute force attacks.
 ...
 * <ul>
 * <li>POST /api/v1/admin/auth/login</li>
 * </ul>
 * </p>
 *
 * <p>
 ...
 * <ul>
 * <li>Login: By client IP (configurable)</li>
 * <li>Blocking: After configurable number of failures</li>
 * </ul>
 * </p>
 *
 * <p>
 ...
 * <ul>
 * <li>HTTP 429 Too Many Requests when limit exceeded</li>
 * <li>Retry-After header with seconds to wait</li>
 * <li>Pass-through for non-targeted endpoints</li>
 * </ul>
 * </p>
 *
 * <p>
...
public class AdminRateLimitFilter implements Filter {
    ...
    public AdminRateLimitFilter(AdminRateLimitProperties properties) {
        ...
        logger.info("AdminRateLimitFilter initialized with login limit: {} requests per {} minutes",
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (shouldApplyRateLimit(requestUri, requestMethod)) {
            ...
            if (isBlocked(clientId)) {
                ...
                logger.warn("Blocked IP attempting login: {} - Retry after {} seconds", clientId, retryAfter);
            ...
            if (!result.allowed) {
                logger.warn("Rate limit exceeded for IP: {} - Retry after {} seconds", 
                        clientId, result.retryAfterSeconds);
                ...
                res.setHeader("X-Rate-Limit-Window", String.valueOf(properties.getLogin().getWindowMinutes()));
                return;
            }
        }
    ...
    private boolean isValidIP(String ip) {
        ...
        return ip.matches(ipv4Pattern) || ip.matches(ipv6Pattern) || ip.equals("::1") || ip.equals("0:0:0:0:0:0:0:1");
    }
     ...
     * May trigger IP blocking if threshold is exceeded.
     *
     * @param clientId the client identifier
     */
    public void recordFailedAttempt(String clientId) {
        ...
        if (failures >= blockAfterFailures) {
            ...
            logger.warn("IP BLOCKED due to {} failed attempts: {} - Blocked for {} minutes",
                    failures, clientId, properties.getLogin().getBlockDurationMinutes());
        }
    }

    /**
     * Records a successful authentication attempt for a client.
     * Clears failure count and any blocks.
     *
     * @param clientId the client identifier
     */
