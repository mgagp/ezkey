 ...
 *
 * <p>This service ensures that auth attempts are created and committed to the database in a
 * separate transaction before the passwordless authentication flow begins waiting. This is critical
 * because AuthAttemptWaitService uses NOT_SUPPORTED propagation and needs to see the committed auth
 * attempt.
 *
 * <p><b>Transaction Flow:</b>
 *
 * <ol>
 *   <li>Parent transaction: AdminAuthService.authenticatePasswordless() [@Transactional]
 *   <li>Helper transaction: createAuthAttempt() [REQUIRES_NEW - commits immediately]
 *   <li>No transaction: AuthAttemptWaitService.waitForResponse() [NOT_SUPPORTED]
 * </ol>
 *
 * <p>Without this helper, the wait service cannot see the auth attempt because the parent
...
public class AdminAuthAttemptTxHelper {
   ...
   *
   * <p>This method creates an authentication attempt in a REQUIRES_NEW transaction that commits
   * before returning. This ensures the auth attempt is visible to the waitForResponse() method
   * which uses NOT_SUPPORTED propagation.
   *
  ...
  public AuthAttemptCreateResponse createAuthAttempt(AuthAttemptCreateRequest request) {
    logger.debug(
        "🔧 Creating auth attempt in independent transaction (enrollment: {})",
