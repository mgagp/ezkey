...
public class TenantService {
   ...
   * tenantName} is changed, uniqueness is validated. Audit fields {@code updatedAt} and {@code
   * updatedByAdmin} are set automatically.
   *
  ...
  public Tenant updateTenant(
      Integer tenantId, TenantUpdateRequestDto request, AdminPrincipal principal) {
    ...
    if (request.tenantName() != null && !request.tenantName().equals(tenant.getTenantName())) {
      if (tenantRepository.existsByTenantNameAndTenantIdNot(request.tenantName(), tenantId)) {
        throw new IllegalArgumentException("Tenant name already exists: " + request.tenantName());
      }
