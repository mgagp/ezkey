 ...
 *
 * <p><b>Usage Pattern:</b> Use factory methods {@link #success} for successful resets and
 * {@link #error} for error responses.
 *
...
public record EnrollmentResetResponseDto(
    @Schema(description = "Indicates if the reset was successful", example = "true")
        Boolean success,
    @Schema(description = "The enrollment ID", example = "123")
        Integer enrollmentId,
    @Schema(description = "New enrollment proof token for binding",
    ...
    @Schema(description = "Integration ID for reference", example = "456")
   ...
   *
   * <p>Creates a success response with new enrollment credentials. The old device is unbound
   * and these credentials must be used to bind a new device.
   *
   ...
   *
   * <p>Creates an error response with a descriptive message. No enrollment credentials are
