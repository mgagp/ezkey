 ...
 *
 * <ul>
 * <li><b>Challenge Flow:</b> Used when challengeRequested=true in login. Client
 * receives both
 * authAttemptId and challengeCode from /login endpoint, displays the challenge
 * to the user,
 * then calls this endpoint with both values to wait for device approval.
 * <li><b>Non-Blocking Flow:</b> Used when nonBlocking=true in login. Client
 * receives only
 * authAttemptId from /login endpoint and calls this endpoint with authAttemptId
 * only (no
 * challengeCode) to poll for device response.
 * </ul>
 *
 * <p>
 * <b>Security (Challenge Flow):</b> The challengeCode acts as proof that the
 * client legitimately
 * initiated the authentication request, preventing enumeration attacks on
 * authAttemptId. Only the
 * client that received the challenge from /login can proceed.
 *
 * <p>
 * <b>Anti-Enumeration Protection:</b> In challenge flow, without the
 * challengeCode requirement,
 * an attacker could enumerate authAttemptId values (1, 2, 3...) and attempt to
 * hijack ongoing
 * authentication attempts. The challengeCode provides cryptographic proof of
 * legitimacy with
 ...
 *                      (required)
 * @param challengeCode Challenge code from the login response (optional -
 *                      required only for
