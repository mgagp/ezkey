 ...
 *
 * <p>
 * This utility creates enrollment files in the demo-device container's data
 * directory, making
 * enrollments immediately available for manual testing and demos without
 * requiring manual
 * enrollment through the web UI.
 *
 * <p>
 * <b>Usage Context:</b> Called after successful bootstrap to sync the initial
 * global admin
 * enrollment to the demo-device application.
 *
 * <p>
 * <b>File Format:</b> Creates JSON files matching
 * {@code EnrollmentStoreService.Record}
 * structure at {@code /app/data/enrollments/{enrollmentId}.json} inside the
...
public class DemoDeviceEnrollmentWriter {
    ...
    private boolean containerIsRunning(String containerName) {
        try {
            ProcessBuilder processBuilder = new ProcessBuilder("docker", "inspect", "--format", "{{.State.Running}}",
                    containerName);
            processBuilder.redirectErrorStream(true);
    ...
    /**
     * Writes enrollment JSON file to demo-device container after successful
     * bootstrap.
     *
     * <p>
     * Retrieves enrollment and integration details from Admin API, constructs JSON
     * matching
     * EnrollmentStoreService.Record format, and writes to demo-device container
     * using docker exec.
     *
     * <p>
     * <b>Throws exception if write fails</b> - bootstrap must fail if demo-device
     * sync fails.
     *
    ...
    public void writeEnrollmentFile(
        ...
        try {
            ...
            Boolean authAttemptChallengeRequired = enrollmentResponse.jsonPath()
                    .getBoolean("authAttemptChallengeRequired");
     ...
     *
     * <p>
     * This method is used for idempotence - to check if bootstrap-init has already
     * created the
     * enrollment file.
     *
    ...
    private boolean fileExistsInContainer(String filePath) {
        try {
            ...
            ProcessBuilder processBuilder = new ProcessBuilder("docker", "exec", containerName, "sh", "-c", command);
            processBuilder.redirectErrorStream(true);
     ...
     *
     * <p>
     * Creates directory with correct permissions (spring:spring user) to match the
     * application
     * runtime user. Fixes permissions on parent directory if needed (volume may
     * have been created
     * with root ownership).
     *
    ...
    private void ensureDirectoryExists() {
        try {
            ...
            String fixParentPermissionsCommand = "mkdir -p /app/data && chown -R spring:spring /app/data && chmod 755 /app/data";
            ProcessBuilder fixParentBuilder = new ProcessBuilder(
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(fixParentProcess.getInputStream()))) {
                String line;
                while ((line = reader.readLine()) != null) {
     ...
     *
     * <p>
     * Uses stdin redirection: writes JSON to stdin of a shell command that creates
     * the file.
     * The container runs directly as the spring user, so no privilege escalation is
     * needed.
     *
     * @param filePath    Path to file inside container
    ...
    private void writeFileToContainer(String filePath, String jsonContent) {
        try {
            ...
            ProcessBuilder processBuilder = new ProcessBuilder("docker", "exec", "-i", containerName, "sh", "-c",
                    command);
            processBuilder.redirectErrorStream(true);
