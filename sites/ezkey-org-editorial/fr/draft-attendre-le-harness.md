---
status: draft
audience: "Développeurs et leads qui suivent l’évolution des agents IA ; retour d’expérience personnel, voix en je, lane « Craft & engineering ». Ton sobre, court (lecture ~3–4 min), ancré dans le vécu Ezkey. Outils nommés comme faits situés (Cursor, Grok Bot, Docker) — pas de pitch produit. Voix québécoise naturelle (registre de Marc en conversation), sans joual ni caricature ; éviter les formulations « je ne méprise pas » / FR-soutenu qui ne collent pas."
...
- Decision 2026-09-17: Quebec voice pass; remove « Pas par mépris ».
...
Sous la loupe d’Ezkey, c’est devenu concret. Pas juste une théorie de plus. Une façon de travailler que j’attendais, sans vouloir la forcer trop vite.

## Une progression que j’ai suivie à mon rythme

Dans Cursor, on a commencé avec des agents dans le chat, quelques prompts, un peu de code. Puis le mode plan, pour ne plus perdre l’analyse dans le fil. Ensuite plusieurs agents sur des worktrees. Plus récemment, les builds cloud.
...
L’héritage de l’IDE est encore là : fenêtres, worktrees, une coordination qui reste proche du mode développement classique. La suite logique, telle que je la vis maintenant avec Grok Bot, c’est un cran au-dessus : une équipe de collaborateurs spécialisés — sécurité, déploiement, produit, ingénierie, mobile, tests — chacun avec sa mémoire, son ordinateur virtuel, et la possibilité de lancer du travail cloud. Ils peuvent se parler entre eux. Ce n’est plus juste un agent dans un onglet ; c’est une forme d’abstraction et de travail récurrent que je n’avais pas vraiment eue avant.

## Pourquoi j’ai tardé volontairement

Début 2025, il y avait déjà plein de nouveautés autour du multi-agents et des skills dans l’écosystème d’Ezkey. J’ai volontairement retardé l’adoption des couches les plus structurantes. Pas parce que ça ne m’intéressait pas. Pour garder la charge cognitive basse et avancer le concret du projet pendant que l’industrie accélérait.
...
Je commence à peine à exploiter ces collaborateurs. J’aurai plus à dire. Déjà, un détail d’intégration m’a surpris : avec Grok Bot ouvert sur mon poste, je peux depuis le téléphone lancer certains jobs qui s’exécutent là-bas. L’équipe suit sur les deux appareils ; je n’ai plus besoin d’être physiquement devant l’écran pour piloter. Il y a peu, mon flux s’arrêtait au bureau. Là, le harness commence à tenir un mode distribué que je n’osais pas espérer si vite.

Le mode projet que je vois apparaître dans Cursor, pour l’instant c’est juste un soupçon — je ne l’ai pas encore vraiment utilisé. Ça me semble une porte d’entrée possible vers une prise de décision encore plus déplacée vers cette couche de coordination. Je préfère le nommer comme hypothèse plutôt que comme vécu.

Ce que je peux déjà dire : le pattern se tient. Worktrees et cloud pour le parallèle ; Grok Bot pour l’intention, la continuité entre spécialistes, et maintenant un fil de pilotage qui traverse le téléphone et le poste. Sous Ezkey, c’est la réalisation concrète de ce que j’attendais d’un mode agent distribué. Il est temps d’embarquer — sans précipiter la suite, et sans faire comme si le récit était terminé.
