...
/**
 * Spring configuration that activates API key rate limiting for the M2M API.
 *
 * <p>Active when {@code ezkey.api-key.rate-limit.enabled=true} (the default). When this
 * configuration is active, {@link org.ezkey.m2m.security.RateLimitService} is created by its own
 * {@code @Service} + {@code @ConditionalOnProperty} annotation.
 *
...
public class ApiKeyRateLimitConfig {
  ...
  public ApiKeyRateLimitConfig() {
    logger.info("M2M API Key Rate Limiting is ENABLED");
  }
}
