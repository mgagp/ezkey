 ...
 * Description: Rate limiting service for M2M API key operations using Bucket4j + Caffeine.
 */
...
/**
 * Rate limiting service for M2M API key operations.
 *
 ...
 * M2M module package. Extraction to a shared module is deferred to a future phase.
 *
...
public class RateLimitService {
  ...
  public RateLimitService(ApiKeyRateLimitProperties properties, MeterRegistry meterRegistry) {
    ...
    logger.info("M2M RateLimitService initialized");
    logger.info(
  ...
  public boolean canCreateAuthAttempt(String apiKeyId) {
    ...
    meterRegistry
        .counter("rate_limit.checks.total", "operation", "create_auth_attempt", "type", "m2m")
        .increment();
    if (!allowed) {
      meterRegistry
          .counter("rate_limit.exceeded.total", "operation", "create_auth_attempt", "type", "m2m")
          .increment();
    }
    return allowed;
  }
  ...
  public boolean canWaitAuthAttempt(String apiKeyId) {
    ...
    meterRegistry
        .counter("rate_limit.checks.total", "operation", "wait_auth_attempt", "type", "m2m")
        .increment();
    if (!allowed) {
      meterRegistry
          .counter("rate_limit.exceeded.total", "operation", "wait_auth_attempt", "type", "m2m")
          .increment();
    }
    return allowed;
  }
