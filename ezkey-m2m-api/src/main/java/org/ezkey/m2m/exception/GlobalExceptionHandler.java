 ...
 * Description: Global exception handler for the M2M API, providing RFC 9457 ProblemDetail
 *              responses for all handled exceptions.
 */

package org.ezkey.m2m.exception;
...
/**
 * Global exception handler for the Ezkey M2M REST API.
 *
 * <p>Provides RFC 9457 ProblemDetail responses for all exceptions thrown in the M2M API layer. This
 * handler ensures consistent, structured error responses for API key consumers (machine-to- machine
 * integrations).
 *
...
public class GlobalExceptionHandler {
   ...
   *
   * <p>Triggered when an M2M client attempts to create an auth attempt for an enrollment that is no
   * longer active (deactivated or revoked). This is a security-relevant event and is logged at WARN
   * level. The caller possesses a valid API key and a known enrollmentId, but the enrollment has
   * been administratively disabled — this may indicate a stale integration or a compromised API key
   * attempting to access a revoked enrollment.
   *
  ...
  public ResponseEntity<ProblemDetail> handleEnrollmentInactiveException(
    ...
    logger.warn("M2M auth attempt blocked: enrollment inactive. Path: {}", request.getRequestURI());
  ...
  public ResponseEntity<ProblemDetail> handleGenericException(
    ...
    logger.error("Unhandled exception in M2M API: {}", ex.getMessage(), ex);
    return buildProblemDetail(
