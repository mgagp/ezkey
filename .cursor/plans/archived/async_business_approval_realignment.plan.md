...
overview: |
  Two-phase roadmap for contextual authentication. Phase 1 (now): keep auth attempts short-term, simplify contextual fields to title+message only, add optional shorter TTL at creation. Phase 2 (future): introduce a separate Business Approval concept with its own entity, async workflow, and webhook mechanism.
todos: []
isProject: false
---
