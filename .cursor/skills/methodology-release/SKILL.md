...
- **Exit when:** bump level is justified, release note is drafted (or written), version properties
  are updated, and the release-notes index row is ready.
- **Call next:** site build/preview, then Cloudflare deploy per `product-docs/site/HANDOFF-PHASE-4.md`.
...
- Changes since last release: `product-docs/methodology/`, `product-docs/templates/`, new or updated
  `.cursor/skills/`, public methodology decisions
...
2. Scan git history or recent files under `product-docs/methodology/`, `product-docs/templates/`,
   and `.cursor/skills/` since the last `released` date.
3. List user-visible changes for an external reader (not Ezkey product delivery from `global/`).
4. Recommend the bump level and next version number.
5. Instantiate `product-docs/templates/methodology-release-note.template.md` as
   `methodology/release-notes/YYYY-MM-DD-<semver>.md`.
6. Update `methodology-version.properties` (`version`, `released`).
7. Add a row to `methodology/release-notes/README.md` (newest first).
8. Remind the operator to build/preview the site and verify the version badge.
