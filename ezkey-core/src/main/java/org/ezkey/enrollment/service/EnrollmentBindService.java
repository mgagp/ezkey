 ...
 * establishes a secure connection with an integration. It implements the
 * read-once guarantee security principle and validates enrollment proof tokens.
 * </p>
 *
 * <p>
 * <b>Security Features:</b>
 * <ul>
 * <li><b>Read-Once Guarantee:</b> Each enrollment can only be bound once</li>
 * <li><b>Proof Token Validation:</b> Prevents unauthorized enrollment access</li>
 * <li><b>State Validation:</b> Ensures enrollment is in correct state for binding</li>
 ...
 * data consistency during the enrollment binding process.
 * </p>
 *
 * <p>
...
public class EnrollmentBindService {
     ...
     */
    public EnrollmentBindService(EnrollmentRepository enrollmentRepository,
    ...
    public EnrollmentBindResponse bind(EnrollmentBindRequest request) {
        logger.info("Starting enrollment bind process for enrollment ID: {}", request.getEnrollmentId());
     ...
     * provided enrollment ID matches the proof token to prevent unauthorized access.
     * </p>
     *
    ...
    private Enrollment validateEnrollment(EnrollmentBindRequest request) {
        logger.debug("Step 1: Performing read-only pre-checks with proof token validation for enrollment ID: {}", request.getEnrollmentId());
        ...
        if (enrollment == null) {
            logger.warn("Validation failed: Enrollment not found or invalid proof token for ID: {}", request.getEnrollmentId());
            throw new IllegalArgumentException("Enrollment binding failed");
        }
        
        logger.debug("Enrollment found with valid proof token: ID={}, Status={}, IntegrationId={}", 
                    enrollment.getEnrollmentId(), enrollment.getStatus(), enrollment.getIntegrationId());

        // Short-circuit: if already processed, avoid acquiring a lock
        if (enrollment.getStatus() != EnrollmentStatus.CREATED) {
            logger.warn("Validation failed: Enrollment already processed - ID: {}, Status: {}", 
     ...
     * <p>
     * This method retrieves the integration associated with the enrollment
     ...
     * falling back to the first available language if the requested language is not found.
     * </p>
     *
     ...
     * <p>
     * This method attempts to find the integration description in the requested language,
     * falling back to the first available language if the requested language is not found.
     * </p>
     *
     * @param integration the integration entity
    ...
    private String resolveIntegrationDescription(Integration integration, String language) {
        ...
        if (integration.getI18n() != null && !integration.getI18n().isEmpty()) {
            ...
            if (description.isPresent()) {
                logger.debug("Found integration description for language {}: {}", language, description.get());
    ...
    private Enrollment acquireLockAndValidate(EnrollmentBindRequest request) {
        ...
        Enrollment enrollment = enrollmentRepository
            .findAndLockUnreadById(request.getEnrollmentId())
            .orElse(null);
            
        if (enrollment == null) {
            logger.warn("Validation failed: Enrollment not found or already bound after lock acquisition for ID: {}", request.getEnrollmentId());
        ...
        if (!enrollment.getEnrollmentProofToken().equals(request.getEnrollmentProofToken())) {
            logger.warn("Validation failed: Proof token mismatch after lock acquisition for ID: {}", request.getEnrollmentId());
            throw new IllegalArgumentException("Enrollment binding failed");
        }
        ...
        if (enrollment.getStatus() != EnrollmentStatus.CREATED) {
            logger.warn("Validation failed: Enrollment already processed after lock - ID: {}, Status: {}", 
                       enrollment.getEnrollmentId(), enrollment.getStatus());
            throw new IllegalStateException("Enrollment already bound by a device");
        }
     ...
     * <p>
     * This method updates the enrollment status to BOUND, ensuring that
     * the enrollment can only be bound once.
     * </p>
     *
     ...
     * integration information, and proof token for the mobile device.
     * </p>
     *
     ...
     */
    private EnrollmentBindResponse buildBindResponse(Enrollment enrollment, Integration integration, 
                                                   String integrationName, String integrationDescription) {
