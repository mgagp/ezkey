 ...
 *
 * <p>
 * This repository provides data access methods for managing bearer tokens used
 * for administrator
 * authentication in the Ezkey system. It includes methods for finding tokens by
 * various criteria
...
public interface AdminTokenRepository extends JpaRepository<AdminToken, Integer> {
   ...
   *
   * <p>
   * This method is used for token validation during API authentication to find
   * the token record
   ...
   *
   * <p>
   * This method is used in filters where Hibernate session may be closed. It uses
   * JOIN FETCH to
   * eagerly load the associated admin to avoid LazyInitializationException.
   *
   * @param bearerToken the bearer token string
   * @return Optional containing the active token with admin loaded, empty
   *         otherwise
   */
  @Query("SELECT t FROM AdminToken t JOIN FETCH t.admin WHERE t.bearerToken = :bearerToken AND"
      + " t.active = true")
  Optional<AdminToken> findByBearerTokenAndActiveTrueWithAdmin(
      @Param("bearerToken") String bearerToken);

  /**
   * Finds an active token by its bearer token string with admin, tenant, and
   * integration eagerly
   * loaded.
   *
   * <p>
   ...
   * LazyInitializationException. This is needed for creating AdminPrincipal with
   * complete scope
   * information.
   *
   * @param bearerToken the bearer token string
   * @return Optional containing the active token with admin, tenant, and
   *         integration loaded, empty
   *         otherwise
   */
  @Query("SELECT t FROM AdminToken t "
   ...
   *
   * <p>
   * This method is used to find all tokens belonging to a specific administrator
   * for token
   ...
   *
   * <p>
   * This method is used to find tokens for administrators of a specific type for
   * security
   ...
   *
   * <p>
   * This method is used to find only active tokens for administrators of a
   * specific type for
   * security monitoring.
   *
   * @param adminType the type of administrator
   ...
   *
   * <p>
   * This method is used to find tokens for administrators belonging to a specific
   * tenant for
   ...
   *
   * <p>
   * This method is used to find only active tokens for administrators belonging
   * to a specific
   ...
   *
   * <p>
   * This method is used to find tokens for administrators managing a specific
   * integration for
   ...
   *
   * <p>
   * This method is used to find only active tokens for administrators managing a
   * specific
   ...
   *
   * <p>
   * This method is used for token cleanup to find tokens that have expired and
   * should be removed
   * from the system.
   *
   ...
   *
   * <p>
   * This method is used for token cleanup to find active tokens that have expired
   * and should be
   * deactivated.
   *
   * @param currentTime the current timestamp
   ...
   *
   * <p>
   * This method is used for security monitoring to find tokens that have not been
   * used for a
   * specified period of time.
   *
   * @param lastUsedBefore the timestamp before which tokens should not have been
   *                       used
   ...
   *
   * <p>
   * This method is used for token cleanup to deactivate tokens that have expired,
   * marking them
   ...
   */
  @Modifying
  @Query("UPDATE AdminToken t SET t.active = false WHERE t.expiresAt < :currentTime AND t.active ="
      + " true")
   ...
   *
   * <p>
   * This method is used for security purposes to deactivate all tokens belonging
   * to a specific
   ...
   */
  @Modifying
  @Query("UPDATE AdminToken t SET t.active = false WHERE t.admin.adminId = :adminId AND t.active ="
      + " true")
   ...
   *
   * <p>
   * This method is used for security monitoring to count how many active tokens a
   * specific
   * administrator has.
   *
   * @param adminId the ID of the administrator
   ...
   *
   * <p>
   * This method is used for security monitoring to count how many active tokens
   * exist for
   ...
   *
   * <p>
   * This method is used for system monitoring to count how many tokens have
   * expired and need
   * cleanup.
   *
   * @param currentTime the current timestamp
   ...
   *
   * <p>
   * This method is used for token cleanup to remove tokens that are both expired
   * and inactive
   * from the database. Active tokens are preserved even if expired for audit
   * trail purposes.
   *
   ...
   *
   * <p>
   * This method is used for monitoring to count how many tokens are eligible for
   * cleanup
  ...
  /**
   * Deactivates all active tokens for administrators belonging to a specific
   * tenant.
   *
   * <p>
   * This method is used during tenant deactivation to revoke all active tokens
   * for
   * administrators in that tenant, effectively logging them all out.
   *
   * @param tenantId the ID of the tenant
   * @return the number of tokens deactivated
   */
  @Modifying
  @Query("UPDATE AdminToken t SET t.active = false WHERE t.tenant.tenantId = :tenantId AND t.active ="
      + " true")
