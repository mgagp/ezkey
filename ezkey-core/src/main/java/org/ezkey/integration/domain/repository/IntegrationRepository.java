...
public interface IntegrationRepository
   ...
   *
   * <p><b>Mutability note:</b> This method returns a fully tracked {@code Integration} entity. Use
   * it only when you need to <em>persist changes</em> to the integration itself (e.g., bootstrap
   * creation). For all read-only access to the system integration, use {@link
   * #findSystemIntegrationReadOnly()} instead to avoid Hibernate dirty-checking and
   ...
   * controllers) to avoid loading a tracked {@code Integration} entity into the session. Loading a
   * tracked entity in the controller layer, and then loading the same integration again inside a
   * {@code @Transactional} service, can produce two managed instances sharing the same {@code
  ...
  /**
   * Fetches the active system integration with its {@code tenant} association eagerly loaded, in
   * <strong>read-only</strong> mode.
   *
   * <p>Use this variant in all code paths that only <em>read</em> the system integration (e.g.,
   * provisioning of new admin enrollments, bootstrap existence check). Read-only mode prevents
   * Hibernate from tracking the {@code Integration.i18n} collection for dirty-checking and cascade
   * operations, eliminating shared-reference conflicts under concurrent load.
   *
   * <p>Note: {@code i18n} is NOT fetched here because callers only need {@code getId()} or {@code
   * getTenant()}. Use {@link #findByIdWithI18nAndTenant(Integer)} when i18n data is needed.
   *
   * <p>Use the standard {@link #findByIsSystemIntegrationAndActiveTrue(Boolean)} only when you
   * intend to <em>persist changes</em> to the system integration entity itself.
   *
   * @return the active system integration with tenant loaded, or empty if not found
   * @since 2025
   */
  @Query(
      "SELECT i FROM Integration i"
