 ...
 * awaiting authentication responses from mobile devices. It implements efficient
 * polling with configurable timeouts and prevents long-running transactions.
 * </p>
 *
 * <p>
 * <b>Key Features:</b>
 * <ul>
 * <li><b>Configurable Timeouts:</b> Customizable timeout and polling intervals</li>
 * <li><b>Efficient Polling:</b> Short-lived transactions to prevent resource exhaustion</li>
 * <li><b>Status Calculation:</b> Intelligent status determination based on attempt state</li>
 * <li><b>Supersession Detection:</b> Handles newer authentication attempts appropriately</li>
 * <li><b>Expiration Handling:</b> Proper handling of expired authentication attempts</li>
 * </ul>
 * </p>
 *
 * <p>
 * <b>Transaction Management:</b>
 * This service uses NOT_SUPPORTED propagation to avoid holding long-running
 * transactions while maintaining data consistency through short-lived operations.
 * </p>
 *
 * <p>
 * <b>Performance Considerations:</b>
 * <ul>
 * <li><b>Short Transactions:</b> Each poll uses a brief transaction to prevent blocking</li>
 * <li><b>Efficient Queries:</b> Optimized database queries for status checking</li>
 * <li><b>Resource Management:</b> Proper cleanup and timeout handling</li>
 * </ul>
 * </p>
 *
 * <p>
...
public class AuthAttemptWaitService {
     ...
     * <p>
     * This method implements efficient polling without holding long-running transactions.
     * It checks the authentication attempt status at regular intervals until completion,
     * timeout, or supersession occurs.
     * </p>
     *
    ...
    public AuthAttemptWaitResponse waitForResponse(Integer authAttemptId, AuthAttemptWaitRequest request) {
        logger.debug("Starting wait operation for auth attempt {} with timeout={}s, polling={}s", 
                    authAttemptId, request.getTimeout(), request.getPolling());
        ...
        while (true) {
            ...
            AuthAttempt authAttempt = authAttemptRepository.findById(authAttemptId)
                .orElseThrow(() -> new ResourceNotFoundException("Authorization attempt", authAttemptId));
            ...
            if (newerAttempt.isPresent()) {
                ...
                logger.info("Auth attempt {} superseded by newer attempt {} for enrollment {} during wait operation", 
                           authAttemptId, newerAttempt.get().getAuthAttemptId(), authAttempt.getEnrollmentId());
            ...
            if (isAttemptCompleted(authAttempt)) {
                int waitDuration = (int) ((System.currentTimeMillis() - startTime) / 1000);
                logger.info("Auth attempt {} completed with status {} after {}s ({} polls)", 
                           authAttemptId, authAttempt.getAuthAttemptStatus(), waitDuration, pollCount);
     ...
     * calculated status, and wait operation metadata.
     * </p>
     *
