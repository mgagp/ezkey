 ...
 *
 * <p>This domain object represents the request data sent by mobile devices when responding to
 * authentication challenges. It contains the cryptographically signed proof tokens and user
 * decisions that complete the authentication flow, providing both security validation and user
 * consent information.
 *
 * <p><b>Usage Context:</b> Used by the AuthAttemptService when mobile devices submit responses to
 * pending authentication attempts. The service layer transforms API DTOs into this domain object
 * for business logic processing and cryptographic validation.
 *
 * <p><b>Authentication Flow:</b> This request represents the final step in the authentication
 * challenge-response flow. After receiving a pending authentication attempt, the mobile device
 * processes the challenge, obtains user consent, and submits this response with cryptographic proof
 * of device authenticity.
 *
 * <p><b>Security Model:</b> Contains device-signed proof tokens that validate both device
 * authenticity and challenge integrity. The cryptographic signature ensures that only the
 * legitimate enrolled device can respond to authentication challenges and prevents replay attacks
 * or unauthorized responses.
 *
 * <p><b>User Consent:</b> Captures explicit user decisions (accept/deny) along with any additional
 * challenge responses required by the authentication policy. This ensures proper user authorization
 * and supports multi-factor authentication scenarios where additional verification steps are
 * required.
 *
...
public class AuthAttemptRespondRequest {
   ...
   *
   * <p>Must reference an existing pending authentication attempt. This ID links the response back
   * to the original authentication challenge and ensures proper tracking throughout the
   * authentication flow. Used by the service layer to locate and update the corresponding
   ...
   *
   * <p>Contains the authentication challenge proof token that has been cryptographically signed by
   * the mobile device using its private key. This signature provides proof of device authenticity
   * and ensures that only the legitimate enrolled device can respond to authentication challenges,
   ...
   *
   * <p>Numeric value representing the response to additional authentication challenges when
   * multi-factor authentication is required. This may include biometric verification results, PIN
   * validation, or other challenge-specific data as determined by the authentication policy and
   * risk assessment. May be null when no additional challenges are required.
   */
   ...
   *
   * <p>Boolean flag indicating whether the user has accepted (true) or denied (false) the
   * authentication attempt. This represents the user's explicit consent and is a critical component
   * of the authentication decision. Must be provided for all authentication attempts to ensure
