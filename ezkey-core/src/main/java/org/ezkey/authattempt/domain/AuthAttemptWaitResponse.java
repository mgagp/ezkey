 ...
 *
 * <p>This domain object represents the response from the authentication wait operation that
 * provides the final authentication status after polling for completion. It includes both the raw
 * authentication attempt data and calculated status information for easy consumption by the service
 * layer.
 *
 * <p><b>Usage Context:</b> Used by the service layer to return authentication wait results. This
 * domain object is mapped to the corresponding DTO and contains the business logic data for
 * authentication status and polling results.
 *
