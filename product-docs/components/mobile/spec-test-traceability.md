...
| --- | --- | --- | --- | --- |
| [`F-enrollment-bind-verify`](../../global/features-and-phases.md#f-enrollment-bind-verify) | [`functional-flows.md#w-mob-enrollment-wizard`](functional-flows.md#w-mob-enrollment-wizard), [`../../../docs/ENROLLMENT_SIGNATURE_PAYLOAD.md`](../../../docs/ENROLLMENT_SIGNATURE_PAYLOAD.md) | Bind payload signature verified; algorithm mismatch fails closed; EC P-256 key generated on keystore; persist only after verify-result signature. | `yarn test` — `useEnrollmentWizard.test.ts`, `enrollmentContinuity.workflow.test.ts`; integration tests against Auth API in a clean-start stack. | `implemented` |
| [`F-auth-pending-respond`](../../global/features-and-phases.md#f-auth-pending-respond) | [`functional-flows.md#w-mob-pending-check`](functional-flows.md#w-mob-pending-check), [`functional-flows.md#w-mob-respond`](functional-flows.md#w-mob-respond), [`../../../docs/AUTH_ATTEMPT_SIGNATURE_PAYLOAD.md`](../../../docs/AUTH_ATTEMPT_SIGNATURE_PAYLOAD.md) | User-initiated polling only; integration-signed outcome verified before success state; respond signs canonical payload. | `yarn test` — `usePendingAuth.test.ts`, `EnrollmentDetailScreen.test.tsx`, `authenticationContinuity.workflow.test.tsx`; integration tests against Auth API. | `implemented` |
| [`F-mobile-reference-app`](../../global/features-and-phases.md#f-mobile-reference-app) | [`stack-and-architecture.md`](stack-and-architecture.md), [`data-model-and-persistence.md`](data-model-and-persistence.md), [`design-decisions.md#adr-mob-0004--android-app-level-sealed-secrets-for-long-lived-enrollment-values`](design-decisions.md#adr-mob-0004--android-app-level-sealed-secrets-for-long-lived-enrollment-values), [`../../../ezkey_mobile/docs/MOBILE_STACK_AND_ARCHITECTURE.md`](../../../ezkey_mobile/docs/MOBILE_STACK_AND_ARCHITECTURE.md), [`../../../ezkey_mobile/docs/MOBILE_DATA_MODEL.md`](../../../ezkey_mobile/docs/MOBILE_DATA_MODEL.md) | Contract-first client generation flow works; no hand-edited DTOs; Android debug APK installable without Metro; `enrollmentProofToken` and `integrationPublicKey` stay out of AsyncStorage cleartext and are rehydrated through the secure secret delegate; local enrollment id / Keystore / seals are installation-scoped (`deriveLocalEnrollmentId`) so two Auth APIs allocating the same server `enrollment_id` cannot collide. | `yarn generate:api`, `yarn typecheck`, `yarn android:install:debug`, targeted Jest (`localEnrollmentIdentity`, enrollmentStorage collision, `trustZoneIsolation.workflow.test.ts`, `complementaryProtections.workflow.test.ts`); operator Pixel multi-install smoke (2026-07-23). Native AES isolation remains instrumented (`sealSecret_isIsolatedPerInstallationScope_MOB017`), not CI. | `implemented` |
...
| Real-device Maestro campaign (`TB-2026-0002` F1) | [`../../../ezkey_mobile/docs/MOBILE_REAL_DEVICE_CHURN_AND_EVIDENCE.md`](../../../ezkey_mobile/docs/MOBILE_REAL_DEVICE_CHURN_AND_EVIDENCE.md), [`../../../ezkey_mobile/maestro/README.md`](../../../ezkey_mobile/maestro/README.md) | F2a compose (no QR); JUnit queues attempts; Maestro approve / approve-challenge / deny / skip-consume; compact `rca.md` per iteration. F2b camera/QR and unbounded 2 h runs stay out. | Named command: `./ezkey-tests/scripts/run-mobile-real-device.sh` (`-P mobile-real-device-tests`). | `implemented` |

## Coverage Summary

- **Features in scope.** 4 product features plus the real-device Maestro campaign row.
- **Implemented coverage.** 3 product features + campaign harness.
- **In-progress coverage.** 1 (`F-rfc9457-errors`).
...
- **Real-device Maestro campaign (`TB-2026-0002` F1)** — named command `./ezkey-tests/scripts/run-mobile-real-device.sh` (2026-08-26). Residual: F2b camera/QR; deterministic admin-timeout TTL if the stack later exposes one. Owner: mobile maintainers.
- Sealed-secret **JVM** coverage exists (`SealedSecretEnvelopeTest`). Keystore seal isolation is instrumented (`sealSecret_isIsolatedPerInstallationScope_MOB017`), not CI. Jest persist→rehydrate→claim isolation across two trust zones is `trustZoneIsolation.workflow.test.ts`; complementary sign-vs-seal forwarding is `complementaryProtections.workflow.test.ts`. StrongBox `STRONG` remains the physical checklist.
