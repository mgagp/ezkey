...
- Secure enrollment persistence — the current Jest coverage exercises the JavaScript storage layer and the bridge contract mocks, but no dedicated Android-native unit or Robolectric tests exist yet for the app-level Keystore sealed-secret primitives. Owner: mobile maintainers. Next: add native tests for envelope parsing, AAD binding, and Keystore fallback behavior.
- Installation-scoped Keystore alias instrumentation — optional distinct-alias `androidTest` deferred at closeout of `TB-2026-07-20-mobile-installation-scoped-enrollment-identity` (unit + Pixel smoke accepted). Owner: mobile maintainers. Next: add only if alias-encoding regression is suspected.
