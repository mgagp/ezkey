...
| [`F-encryption-key-rotation`](features-and-phases.md#f-encryption-key-rotation) | [Admin API encryption endpoints](../../docs/ENDPOINT.md), [ADR-0008](architecture-decisions.md#adr-0008-tink-keyset-sync-concurrent-read-path) | Rotation creates pending key; re-encryption batches progress; drained verification protects decommissioning; encrypt/decrypt hot path does not serialize on keyset version checks. | Admin API tests; re-encryption flow tests; `TinkKeyManagerConcurrencyTest`; elective `ReencryptionFullTriggerConcurrentActivityElectiveTest`. | `in-progress` |
