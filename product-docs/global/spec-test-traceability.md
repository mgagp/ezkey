...
| [`F-encryption-key-rotation`](features-and-phases.md#f-encryption-key-rotation) | [Admin API encryption endpoints](../../docs/ENDPOINT.md), [ADR-0008](architecture-decisions.md#adr-0008-tink-keyset-sync-concurrent-read-path), [ADR-0011](architecture-decisions.md#adr-0011-tink-native-database-keyset-envelope), [`REENCRYPTION_OPERATIONS.md`](../../docs/REENCRYPTION_OPERATIONS.md) §1.2 | Rotation creates pending key; re-encryption batches progress; drained verification protects decommissioning; encrypt/decrypt hot path does not serialize on keyset version checks; DB keyset synchronization uses Tink's encrypted-keyset envelope directly; re-encryption discovery uses indexed `*_encryption_key_id` equality lookups, not `LIKE` ciphertext scans (`I-2026-0029` / `TB-2026-07-26`). | Admin API tests; re-encryption flow tests; `TinkKeyManagerConcurrencyTest`; `ezkey-core` unit tests (`EncryptionEntityListenerTest`, `ReencryptionServiceTest`); elective `ReencryptionFullTriggerConcurrentActivityElectiveTest`; clean-start DB blob inspection. | `in-progress` |
