...
| Enrollments | `GET /api/v1/enrollments` | A | Which devices/users are enrolled and in what state? | **ID**, name, status (+ warning), **integration name**, user ID, **tenant name** (GA), created, last used | **Done** — order: identity → state → scope → time; batch integration+tenant join | Both (tenant col GA only) | `implemented` | TB `TB-2026-06-18-admin-ui-lists-tier-a-enrollments` |
| API keys | `GET /api/v1/api-keys/...` | A | Which keys exist and for which integration? | **ID**, description, status (+ warning), **integration name**, **tenant name** (GA), created, expires, last used, **revoke action** | Draft — list API join; keep trailing revoke per [`admin-ui-list-quick-security-actions.md`](admin-ui-list-quick-security-actions.md) | Both | `draft` | TB `TB-2026-06-18-admin-ui-lists-tier-a-api-keys` |
