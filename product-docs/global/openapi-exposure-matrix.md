...
- `EXP1` is treated as **prod-like** for raw OpenAPI exposure posture.
...
| **Evaluator (`EXP1`)** | `disabled publicly` | `disabled publicly` | `disabled publicly` | `disabled publicly` | `disabled publicly` when deployed | `disabled publicly` when deployed | `not exposed` | `enabled` | Evaluator access should converge toward the curated portal, not raw host tooling |
...
1. `EXP1` should no longer be treated as a tolerated public exception for raw Swagger/UI exposure.
