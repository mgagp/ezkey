...
- **(K) "Superseded by" / "Supersedes" convention for archived V-\* / TB-\* / I-\* notes.** When an artefact is `archived` because its substance has been **reformed** into successors (rather than abandoned), it should explicitly point to those successors via a `Superseded:` line, and the successor should reciprocate via `Supersedes:`. Used inline on `V-2026-0002` (archived 2026-05-08, superseded by `V-2026-0010`) and on `V-2026-0010` (supersedes `V-2026-0002`) but not yet documented in `nomenclature.md`. Decide whether to formalize as a small subsection in `nomenclature.md` or leave as soft convention with this single precedent.
