...
# Optional base compose marker (labels). Property matrix lives in application-docker-base.properties.
if [ "${EZKEY_RUNTIME_PROFILE:-}" = "base" ] && [ -f "${SCRIPT_DIR}/docker-compose.base.yml" ]; then
    COMPOSE_ARGS="${COMPOSE_ARGS} -f ${SCRIPT_DIR}/docker-compose.base.yml"
    echo "🔧 Base runtime overlay enabled: docker-compose.base.yml"
fi
...
echo "  - Crypto API:   http://localhost:9090/swagger-ui/index.html"
echo ""
...
echo "  - Base (opt-in): EZKEY_RUNTIME_PROFILE=base ./docker/start.sh"
echo "  - Or: ./ezkey-tests/clean-start.sh --runtime=base"
...
echo "  - Login via POST /api/v1/admin/auth/login and approve on demo-device"
echo ""
echo "📋 Retrieve Bootstrap Artifacts (if needed):"
echo "  - Bootstrap credentials: docker run --rm -v ezkey_bootstrap-artifacts:/data alpine cat /data/bootstrap-credentials.json"
echo "  - Device credentials: docker run --rm -v ezkey_bootstrap-artifacts:/data alpine cat /data/device-credentials.json"
echo ""
