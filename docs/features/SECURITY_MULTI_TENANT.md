...
### 6.9 Compatibilité Compilation Native

**Principe :** Assurer que toutes les nouvelles entités et services sont compatibles avec la compilation native GraalVM

**Configuration Native Existante Identifiée :**
- ✅ **Profile native** configuré dans le pom.xml parent
- ✅ **Buildpack** : `paketobuildpacks/builder-jammy-tiny`
- ✅ **Configuration native** : `native-image.properties` avec optimisations AWS Lambda
- ✅ **Métadonnées** : `reflect-config.json`, `resource-config.json`, `serialization-config.json`

**Nouvelles Entités à Ajouter dans la Configuration Native :**

#### A. Nouvelle Configuration Native pour Admin-API
```java
// ezkey-admin-api/src/main/java/org/ezkey/admin/config/AdminNativeConfiguration.java
@Configuration
...
public class AdminNativeConfiguration {
    ...
    static class AdminRuntimeHints implements RuntimeHintsRegistrar {
        ...
        public void registerHints(RuntimeHints hints, ClassLoader classLoader) {
            // Nouvelles entités de sécurité
            hints.reflection()
                    .registerType(org.ezkey.tenant.domain.entity.Tenant.class)
                    .registerType(org.ezkey.admin.domain.entity.EzkeyAdmin.class)
                    .registerType(org.ezkey.adminauth.domain.entity.AdminToken.class)
                    .registerType(org.ezkey.adminauth.domain.entity.AdminTempToken.class);
            
            // DTOs d'authentification
            hints.reflection()
                    .registerType(org.ezkey.adminauth.dto.AdminLoginRequestDto.class)
                    .registerType(org.ezkey.adminauth.dto.AdminLoginResponseDto.class)
                    .registerType(org.ezkey.adminauth.dto.AdminTokenResponseDto.class);
            
            // Sérialisation pour Jackson
            hints.serialization()
                    .registerType(TypeReference.of(org.ezkey.tenant.domain.entity.Tenant.class))
                    .registerType(TypeReference.of(org.ezkey.admin.domain.entity.EzkeyAdmin.class))
                    .registerType(TypeReference.of(org.ezkey.adminauth.domain.entity.AdminToken.class))
                    .registerType(TypeReference.of(org.ezkey.adminauth.domain.entity.AdminTempToken.class));
...
#### B. Mise à Jour des Métadonnées Native

**reflect-config.json - Ajout des Nouvelles Classes :**
```json
[
  // Classes existantes - INCHANGÉES
  {
    "name": "org.ezkey.admin.AdminApplication",
    "methods": [
  ...
  {
    "name": "org.ezkey.tenant.domain.entity.Tenant",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.admin.domain.entity.EzkeyAdmin",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.adminauth.domain.entity.AdminToken",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.adminauth.domain.entity.AdminTempToken",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.adminauth.dto.AdminLoginRequestDto",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.adminauth.dto.AdminLoginResponseDto",
    "allPublicMethods": true,
    "allPublicFields": true
  },
  {
    "name": "org.ezkey.adminauth.dto.AdminTokenResponseDto",
    "allPublicMethods": true,
    "allPublicFields": true
  }
]
...
**resource-config.json - Ajout des Nouvelles Ressources :**
```json
{
  "resources": {
    "includes": [
      ...
      {
        "pattern": "\\Qapplication-native.properties\\E"
      },
      {
        "pattern": "\\Qapplication-*.properties\\E"
      },
      ...
      {
        "pattern": "\\QMETA-INF/native-image/org.ezkey/ezkey-admin-api/*\\E"
      }
    ]
  },
  "bundles": [
...
**serialization-config.json - Ajout des Nouvelles Classes :**
```json
[
  "org.postgresql.Driver",
  "org.hibernate.dialect.PostgreSQLDialect",
  "org.ezkey.tenant.domain.entity.Tenant",
  "org.ezkey.admin.domain.entity.EzkeyAdmin",
  "org.ezkey.adminauth.domain.entity.AdminToken",
  "org.ezkey.adminauth.domain.entity.AdminTempToken"
]
```

#### C. Mise à Jour de la Configuration JPA

**AdminJpaConfig.java - Ajout des Nouveaux Packages :**
```java
@Configuration
@EnableJpaRepositories(
        basePackages = { 
...
#### D. Mise à Jour de l'Application

**AdminApplication.java - Ajout des Nouveaux Packages :**
```java
@SpringBootApplication(
        scanBasePackages = { 
                "org.ezkey.admin", 
...
### 6.10 Vérification de Compatibilité Native

**Tests de Compilation Native :**
```bash
# Test AOT processing
mvn spring-boot:process-aot -pl ezkey-admin-api -Pnative

# Test buildpack native image
mvn spring-boot:build-image -pl ezkey-admin-api -Pnative -DskipTests

# Test direct native compilation (si GraalVM installé)
mvn native:compile -pl ezkey-admin-api -Pnative
```

**Vérifications Requises :**
- ✅ **Reflection** : Toutes les nouvelles entités et DTOs
- ✅ **Sérialisation** : Support Jackson pour les nouvelles classes
- ✅ **Ressources** : Configuration native et propriétés
- ✅ **JPA** : Entités et repositories scannés correctement
- ✅ **Spring Security** : Configuration native compatible
