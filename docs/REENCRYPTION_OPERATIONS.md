...
- **Re-encryption wall time (estimate), key detail** — For a **`DRAINED`** key, `EncryptionKeyResponse.reencryptionWallClockSeconds` is a retrospective, parallelism-aware estimate of how long the full migration off that old key took: **sum of durations for non-sharded batches** + **max duration per sharded group** (grouped by `target_table` + `target_column` + `new_key_id`), mirroring `ReencryptionBatchParallelRunner`'s per-shard mutex model (§9) rather than raw CPU time. Computed in `KeyUsageVerificationService` from `COMPLETED` batches only; `null` when the key is not drained or no batch has both `startedAt` and `completedAt`. Displayed in the key detail dialog alongside algorithm, introduced date, and cumulative re-encrypted count.
...
| `auth-attempt-shard-count` | `4` (Admin config) / `1` (Java) | `1` = disabled (single batch per auth-attempt column). Values **4 / 8 / 16** split `ezkey_auth_attempt` work into that many shards using `mod(auth_attempt_id, N) = shard_index`. **Enrollment** is never sharded. Admin Docker defaults pair **4 workers with 4 shards**. |
