...
| `AUDIT_INTEGRITY_RUPTURE` | `NightlyIntegrityValidationService` detects per-entry HMAC or checkpoint chain rupture in the retroactive nightly window | `"AUDIT_INTEGRITY_RUPTURE:" + windowStartEpochMillis` | `CRITICAL` | Global Admin reconciles via `POST /api/v1/audit-logs/lifecycle/reconcile-integrity-rupture` (reason **`INTEGRITY_RUPTURE_CONCILIATED`**). Blocked while `AUDIT_CHAIN_HEARTBEAT_STALE` is OPEN. |
