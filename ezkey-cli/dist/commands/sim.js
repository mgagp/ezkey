...
class SimCommand {
    ...
    getCommand() {
        const cmd = new commander_1.Command('sim')
            .description('Simulation API commands for cryptographic operations and testing');
        cmd
            .command('prooftoken')
    ...
    async generateKeyPair(keySize) {
        ...
        if (keySize < 1024 || keySize > 4096) {
            console.error(chalk_1.default.red('Error: Key size must be between 1024 and 4096 bits'));
            return;
        }
    ...
    async signData(options) {
        ...
        let data = {};
        if (options.json) {
            data = json_utils_1.JsonUtils.processInput(options.json);
        }
        else {
            ...
            if (options.data.startsWith('@')) {
                ...
                try {
                    ...
                    console.error(chalk_1.default.red(`Error reading data file: ${error instanceof Error ? error.message : String(error)}`));
                    return;
                }
            }
            else {
            ...
            if (options.privateKey.startsWith('@')) {
                ...
                try {
                    ...
                    console.error(chalk_1.default.red(`Error reading private key file: ${error instanceof Error ? error.message : String(error)}`));
                    return;
                }
            }
            else {
    ...
    async validateSignature(options) {
        ...
        if (options.json) {
            ...
            if (options.signature.startsWith('@')) {
                ...
                try {
                    ...
                    console.error(chalk_1.default.red(`Error reading signature file: ${error instanceof Error ? error.message : String(error)}`));
                    return;
                }
            }
            else {
            ...
            if (options.publicKey.startsWith('@')) {
                ...
                try {
                    ...
                    console.error(chalk_1.default.red(`Error reading public key file: ${error instanceof Error ? error.message : String(error)}`));
                    return;
                }
            }
            else {
        ...
        if (response.success && response.data) {
            ...
            if (isValid) {
                ...
                console.log(chalk_1.default.red('✗ Signature is invalid'));
            }
            if (message) {
            ...
            if (this.config.get('prettyPrint')) {
                console.log('\nFull response:');
                console.log(json_utils_1.JsonUtils.formatOutput(response.data, true));
            }
        }
        else {
            this.outputResponse(response);
