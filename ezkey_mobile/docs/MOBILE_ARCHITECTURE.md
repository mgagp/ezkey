...
- **Security References**:
  - [`docs/CRYPTO.md`](../../docs/CRYPTO.md) – cryptographic requirements (Ed25519, 32-byte keys, 64-byte signatures, HKDF-SHA-256)
...
- With Ed25519, device keys are derived on-demand from root key, so no device aliases need to be stored.
...
5. Derives Ed25519 public key for enrollment ID with `cryptoService.getPublicKey(enrollmentId)`.
6. Completes verification through `enrollmentsApi.verify` with Ed25519 signature and persists records with `enrollmentStorage`.
...
1. Ensures root key exists (keys derived on-demand).
2. Builds device proof tokens per poll to prevent replay.
3. Submits Ed25519 signatures generated by `cryptoService.sign(enrollmentId, payload)`.
