...
signing key, while a separate app-level Android Keystore AES key protects sealed-secret envelopes for
`enrollmentProofToken` and `integrationPublicKey` on Android. Other platforms continue to use their secure-storage
