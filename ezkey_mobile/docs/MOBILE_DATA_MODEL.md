...
kept behind the native keystore path and the enrollment proof token is stored through the secure-storage delegate.
`StoredEnrollment` is rehydrated at runtime by combining AsyncStorage metadata with the secure proof token and the
persisted integration verification material needed for later auth flows.
...
| `enrollmentProofToken` | QR payload / bind response | Secure-storage entry keyed by enrollment ID; rehydrated into `StoredEnrollment` at runtime | Used in later `pending` requests | Sensitive enrollment proof material no longer persists in the AsyncStorage collection. |
| `integrationPublicKey` | Bind response | AsyncStorage-backed enrollment metadata | Used for all later integration-signature verification | Public key material, but security-critical. |
...
| Secure storage delegate values | Keychain-backed generic password entries | `react-native-keychain` wrapper | Protects enrollment proof tokens and other small local secrets | The enrollment storage class rehydrates proof tokens from this delegate at read time. |

The trust boundary to keep in mind is that the enrollment proof token now lives in secure storage, the integration
verification material lives in local metadata storage, and the device private key remains off-limits to normal
application code.
...
signing key, while the secure-storage delegate protects smaller application secrets such as `enrollmentProofToken`.
Those are complementary protections, not a single "StrongBox unseals all local app data" design.
