...
- Persisted attributes per enrollment: `enrollmentId`, `integrationId`, `integrationName`, `tenantName`, installation metadata, and local timestamps. Long-lived verification material such as `integrationPublicKey` is rehydrated at runtime rather than left in the plain AsyncStorage metadata collection.
- Uses the native keystore path for the per-enrollment private signing key, an Android app-level `Android Keystore` AES sealing key for long-lived local secrets such as `enrollmentProofToken` and `integrationPublicKey`, and AsyncStorage only for lower-sensitivity enrollment metadata.
...
- For secure storage, prefer platform-backed secure storage abstractions. Do not describe the current app as if one StrongBox-backed private signing key decrypts every other local secret; the private signing key and the separate secret-sealing path follow distinct protection roles today.
