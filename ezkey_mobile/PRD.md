...
sequenceDiagram
    ...
    App->>AuthAPI: POST /api/v1/enrollments/bind (JSON body with enrollmentId + proofToken)
    AuthAPI-->>App: Integration info + enrollmentProofToken
    App->>AuthAPI: POST /api/v1/enrollments/verify (payload signed)
...
sequenceDiagram
    ...
    App->>AuthAPI: POST /api/v1/auth-attempts/pending (body)
...
- `Check pending` button requests `POST /api/v1/auth-attempts/pending` with the stored enrollment payload (ID + proof tokens) and routes to the request screen when data is returned.
...
- QR parsing supports `enrollmentId` and `proofToken` in JSON or query-string format (same schema as Kotlin demo), which are passed in the POST `/api/v1/enrollments/bind` payload.
