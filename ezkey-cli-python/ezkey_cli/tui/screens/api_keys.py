...
TUI Module: API Keys Screen
Description: Screen for listing API keys
"""
...
class ApiKeysScreen(Screen):
  ...
  def on_mount(self) -> None:
    ...
    log.debug("ApiKeysScreen mounted")
  ...
  def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
    ...
    if row_data:
      api_key_id = int(row_data[0])
      from .api_key_detail import ApiKeyDetailScreen
      self.app.push_screen(ApiKeyDetailScreen(api_key_id))
  ...
  def _load_keys(self) -> None:
    ...
    active_only = self.filters.get("active_only", False)

    if integration_id:
      response = api_client.get_api_keys_for_integration(integration_id)
    else:
      response = api_client.get_api_keys()

    if not response:
      log.warning("No API keys response")
      self._set_status("Failed to load API keys")
      table = self.query_one("#table", DataTable)
      table.clear()
    ...
    if active_only:
      self.all_keys = [k for k in self.all_keys if k.get("active")]
  ...
  def _render_page(self) -> None:
    """Render current page of API keys."""
    ...
    for item in page_items:
      ...
      integration_key = item.get("integrationKey", "")
  ...
  def action_create(self) -> None:
    ...
    def on_create_result(result) -> None:
      if isinstance(result, dict) and result.get("created"):
        ...
        integration_key = response.get("integrationKey", "")
        secret_key = response.get("secretKey", "")
        warning = response.get("warning", "")

        self.app.push_screen(
            ApiKeyCreatedModal(integration_key, secret_key, warning),
            lambda _: self._load_keys()
        )
      elif result:
        self._load_keys()

    self.app.push_screen(CreateApiKeyModal(), on_create_result)
  ...
  def action_filter(self) -> None:
    ...
    self.app.push_screen(ApiKeyFilterModal(current_filters=self.filters), on_filter_result)
