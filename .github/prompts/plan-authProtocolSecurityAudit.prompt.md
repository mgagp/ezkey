...
3. **[CRITIQUE] Implémenter une liste de trusted proxies** — Use `ezkey.trusted-proxies.cidrs` / `ezkey.trusted-proxies.required` (not the obsolete `ezkey.rate-limit.trusted-proxies` name). Ne faire confiance aux headers `X-Forwarded-For` que si `remoteAddr` est dans la liste.
